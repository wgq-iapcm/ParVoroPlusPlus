...
int main(int argc, char* argv[])
{
  ...
  int nx=0, ny=0, nz=0;         // voro::container param[in], the number of grid blocks in each of the three coordinate directions
  ...
  if(kdtree)
  {
      ...
      {
          if(rank == 0)
              fprintf(stderr, "ERROR: using k-d tree with wrap on and fewer than 64 blocks is likely to fail\n");
          return 1;
      }
  }
  ...
  double lvol, gvol = 0, dvol = 0;
  if(debug)
  {
      lvol = volume_stat(master);
      world.barrier();
      diy::mpi::all_reduce(master.communicator(), lvol, gvol, std::plus<double>());
  ...
  if(rank == 0) 
  {
      ...
      fprintf(stderr," infile: %s\n outfile: %s\n ranksize: %d\n threads: %d\n tot_blocks: %d\n wrap: %d\n tot_particles: %ld\n",infile.c_str(), outfile.c_str(), world.size(), num_threads, tot_blocks, wrap, tot_particles);
